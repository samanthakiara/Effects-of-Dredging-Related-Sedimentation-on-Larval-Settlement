...
scatter.smooth(data$Trial, rstandard(mod4, type='deviance'), col='gray') #reminder: Trial not in mod4
...
exp_coefficients <- exp(coefficients) - 0.0001 #remember log(sami$Conc+0.0001), so need to subtract
...
pm_plot <- ggplot(mod3meta_eff, aes(x = logConc, y = fit)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) +
#  geom_point(data = sami, aes(x = logConc, y = P_settle), color = "red", shape = 16) +
